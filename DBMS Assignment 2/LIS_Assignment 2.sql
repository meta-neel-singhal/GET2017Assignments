...
SELECT member_id, member_name, address_line1, address_line2, category
FROM `members`;
...
SELECT `title`.title_name, `subjects`.subject_name, `publisher`.publisher_name
FROM `title`
INNER JOIN `subjects`
ON `title`.subject_id = `subjects`.subject_id
INNER JOIN `publisher`
ON `title`.publisher_id = `publisher`.publisher_id;

-- Display number of members in each category.
SELECT category, COUNT(member_name)
FROM members
GROUP BY category;
...
WHERE m2.member_name = 'Neel Singhal';
...
SELECT DATE(bi.issue_date) "Book Issued On", bi.accession_no "Accession Number Of Issued Book", bi.member_id "Book Issued To", IFNULL(DATE(br.return_date), "") "Book Returned On"FROM book_issue bi 
LEFT JOIN book_return br 
ON bi.accession_no = br.accession_no
LEFT JOIN members m
ON m.member_id = bi.member_id
ORDER BY bi.issue_date ASC, m.member_name ASC;