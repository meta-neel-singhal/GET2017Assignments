...
public class Graph<T> implements Node<T> {
	...
	private void initEntity() {
		...
		for (String str : entityList) {
			...
			if ("Organization".equalsIgnoreCase(details[5])) {
				nodesHashMap.put(details[0], (T) new Organization(details[0], details[1],
						details[2], Long.parseLong(details[3]), details[4], details[5]));
			} else {
				nodesHashMap.put(details[0], (T) new User(details[0], details[1],details[2],
						Long.parseLong(details[3]), Integer.parseInt(details[4]), details[5]));
			}
		}
	}
	 ...
	 * 
	 * @param id     Id of user/organization
	 * @param obj    Entity class object;
	 */
